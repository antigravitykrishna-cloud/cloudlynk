...
// Single source of truth: the hosted page at
// supabase/functions/legal-pages/copyright.ts, opened here in an in-app
// browser. Keep this screen a thin launcher so the app and the
// Play-Console-linked page can never drift apart.
export default function CopyrightPolicyScreen() {
  ...
  const open = async () => {
    if (!config.copyrightPolicyUrl) return;
    setOpening(true);
    openedRef.current = true;
    try {
      await WebBrowser.openBrowserAsync(config.copyrightPolicyUrl);
    } finally {
      setOpening(false);
      router.back();
    }
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Copyright &amp; IP Policy</Text>
        <View style={{ width: 60 }} />
      </View>
      <View style={styles.content}>
        {config.copyrightPolicyUrl ? (
          <>
            {opening && <ActivityIndicator color={Colors.accent} size="large" style={{ marginBottom: Spacing.lg }} />}
            <Text style={styles.body}>Opening the Copyright &amp; IP Policy…</Text>
          ...
          <Text style={styles.body}>The Copyright &amp; IP Policy isn't configured yet. Contact support for a copy.</Text>
        )}
      </View>
    </SafeAreaView>
  );
